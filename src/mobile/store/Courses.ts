...
export type KnownAction = FetchCoursesSuccessAction | CourseCreatedSuccessAction | UpdateCourseSuccessAction;

const initialState: CoursesState = { courses: [] };

export const actionCreators = {
  fetchCourses: (filter: string): AppThunkAction<KnownAction> => (dispatch, getState) => {
    const appState = getState();
    if (!appState.user || !appState.user.loggedIn || !appState.user.user) return;
    fetch(`${urls.discmanWebBaseUrl}/api/courses?filter=${filter}`, {
      method: "GET",
      headers: {
  ...
  createCourse: (courseName: string, layoutName: string, numberOfHoles: number): AppThunkAction<KnownAction> => (dispatch, getState) => {
    const appState = getState();
    if (!appState.user || !appState.user.loggedIn || !appState.user.user) return;
    fetch(`${urls.discmanWebBaseUrl}/api/courses`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${appState.user.user.token}`,
  ...
  updateCourse: (course: Course): AppThunkAction<KnownAction> => (dispatch, getState) => {
    const appState = getState();
    if (!appState.user || !appState.user.loggedIn || !appState.user.user) return;
    fetch(`${urls.discmanWebBaseUrl}/api/courses/${course.id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${appState.user.user.token}`,
  ...
  switch (action.type) {
      ...
      return {
        ...
        courses: [
          ...state.courses.map((c) => {
            ...
            if (layout) {
              var newc: [string, Course[]] = [c[0], [...c[1].filter((x) => x.id !== action.course.id), action.course]];
              return newc;
            }
            return c;
          }),
    ...
    case "COURSE_CREATED_SUCCEED":
      if (state.courses.some((c) => c[0] === action.course.name)) {
        ...
        return {
          ...state,
          courses: [...state.courses, [action.course.name, [action.course]] as [string, Course[]]],
        };
      }
