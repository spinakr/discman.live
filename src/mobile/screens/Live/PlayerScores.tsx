  ...
  return (
    <GestureRecognizer style={styles.scoresContainer} onSwipeRight={() => goToPreviousHole()} onSwipeLeft={() => goToNextHole()}>
      <View style={styles.scoresHeaderRow}>
        ...
        <View style={styles.scoresCell}>{prevHole && <Text style={{ fontSize: 8 }}>{prevHole?.hole.number}</Text>}</View>
        <View style={styles.currentScoresCell}>
          <Text style={{ fontSize: 8 }}>{currentHole.hole.number}</Text>
        </View>
        <View style={styles.scoresCell}>{nextHole && <Text style={{ fontSize: 8 }}>{nextHole?.hole.number}</Text>}</View>
      </View>
      {playerScores.map((p) => {
...
const styles = StyleSheet.create({
  ...
  scoresCellHead: { flex: 3, justifyContent: "center" },
  scoresHeadText: {},
